...
const Gallery = () => {
  ...
  const galleryImages = [
    {
      id: 1,
      titleKey: "Beiteen Association 1991",
      categoryKey: "gallery.categories.heritage",
      image: beiteen1991,
      descriptionKey: "Historic photo from our community's early days in 1991",
    },
    {
      id: 2,
      titleKey: "Our History",
      categoryKey: "gallery.categories.heritage",
      image: beiteenHistoric,
      descriptionKey: "Celebrating our roots and the journey of our community",
    },
    {
      id: 3,
      titleKey: "Community Gathering",
      categoryKey: "gallery.categories.events",
      image: communityGathering,
      descriptionKey: "Community members coming together for a local gathering",
    },
    {
      id: 4,
      titleKey: "Board Meeting",
      categoryKey: "gallery.categories.organization",
      image: boardMeeting,
      descriptionKey: "Board members planning and discussing community initiatives",
    },
    {
      id: 5,
      titleKey: "Cultural Celebration",
      categoryKey: "gallery.categories.culture",
      image: culturalCelebration,
      descriptionKey: "Joyful cultural event with traditional decorations",
    },
    {
      id: 6,
      titleKey: "Youth Program",
      categoryKey: "gallery.categories.education",
      image: youthProgram,
      descriptionKey: "Youth participating in educational activities",
    },
  ];
        ...
        <div className="section-container">
          <div className="max-w-6xl mx-auto">
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {galleryImages.map((image, index) => (
                <MotionSection key={image.id} delay={index * 0.05}>
                  <motion.div 
                    className={`bg-card border border-border rounded-lg overflow-hidden group ${isRTL ? 'text-right' : ''}`}
                    whileHover={{ y: -4 }}
                  ...
                  >
                    ...
                    <div className="p-4">
                      <span className={`inline-block px-2 py-1 text-xs font-medium bg-primary/10 text-primary rounded mb-2 ${isRTL ? 'float-right ml-2' : ''}`}>
                        {t(image.categoryKey)}
                      </span>
                      <h3 className="font-heading text-lg font-semibold text-foreground clear-both">
                        {image.titleKey}
                      </h3>
                      <p className="text-sm text-muted-foreground mt-1">
                        {image.descriptionKey}
                      </p>
                    </div>
                  </motion.div>
                </MotionSection>
            ...
            <MotionSection delay={0.3} className="mt-12">
              <div className={`p-8 bg-muted rounded-lg text-center ${isRTL ? 'text-right' : ''}`}>
                <h3 className="font-heading text-xl font-semibold text-foreground mb-2">
                  {t("gallery.morePhotos.title")}
                </h3>
                <p className="text-muted-foreground max-w-lg mx-auto">
                  {t("gallery.morePhotos.desc")}
                </p>
              </div>
            </MotionSection>
          </div>
